...
describe('Test Suite', () => {
  ...
  it('Verificar que la página cargue correctamente', () => {
    ...
    cy.get('[data-cy="input-first-name"]').type('eliminar Endo');
    cy.get('[data-cy="input-last-name"]').type('Endodoncista');
    cy.get('[data-cy="input-dni"]').type(rutAleatorio);
    cy.get('[data-cy="data-cy=input-email"]').type(emailAleatorio);
    cy.get('#password1').type('sB1HZ$iiwB6Ota');
    cy.get('#password2').type('sB1HZ$iiwB6Ota');
    
    cy.get('#instrumentacion_mecanizada-button > .mdc-switch__track').click({force: true});
    cy.get('#obturacion_termoplastica-button > .mdc-switch__track').click({force: true});
