...
cy.get(':nth-child(6) > .fuse-vertical-navigation-item-wrapper > .mat-mdc-tooltip-trigger').click();
cy.get('.text-4xl').should('contain.text', 'Ajustes');
cy.get('.px-6 > :nth-child(2)').click();

cy.get('#security > .ml-3').click();
cy.get('.ml-2').should('contain.text', 'Seguridad');
cy.get('#mat-mdc-form-field-label-10 > .ng-tns-c3736059725-11').should('be.visible');
cy.get('#mat-mdc-form-field-label-12 > .ng-tns-c3736059725-12').should('be.visible');
cy.get('mat-label.ng-tns-c3736059725-13').should('be.visible');
