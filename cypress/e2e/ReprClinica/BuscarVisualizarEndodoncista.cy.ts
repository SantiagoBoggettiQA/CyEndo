...
cy.get('#mat-input-11').click().type('msanz+endodostit@gux.tech');
cy.wait(2000);
cy.get('.justify-end > button > .mat-icon').click({ force: true });
cy.get(':nth-child(4) > .text-2xl').should('contain.text','msanz+endodostit@gux.tech');
  });
